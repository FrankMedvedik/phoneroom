            ...
            if ((ReportDateRange != null) && (RecruiterSIP != null))
            {
                ...
                try
                {
                    var ro = await LyncCallLogSvc.GetLynJobsForRecruiter(ReportDateRange.StartRptDate,
                        ...
                        HeadingText = String.Format("{0} Jobs with call activity between  {1} and {2}", RecruiterJobs.Count, ReportDateRange.StartRptDate, ReportDateRange.EndRptDate);
                        LoadedOk = true;
                }
                catch (Exception e)
                {
